...
export default class ActionStackProvider extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onRedo = (step = 1) => {
    if (this.state.actionStackPointer + step >= this.state.actionStack.length) {
      return;
    } else {
      this.setState((state) => {
        let updatedVal = state.val;
  ...
  render() {
    ...
    if (this.props.onUpdate === undefined) data.value = this.state.val;
    return (
      <ActionStackContext.Provider value={data}>
        {this.props.children}

...
import { ActionStackContext } from "../constants/MyContexts";

export default class ProviderObject extends Component {
  static contextType = ActionStackContext;
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleSubmit = (event) => {
    ...
    console.log(this.state.toAction);
    this.context.onAction(this.state.toAction);
    this.setState({ toAction: {}, text: { dev: "", qa: "" } });
  };

  render() {
    const { onUndo, onRedo, value } = this.context;
    return (
      <div style={{ padding: "50px 50px" }}>
        {" "}
        <span>ActionStackProvider</span>
        <div
